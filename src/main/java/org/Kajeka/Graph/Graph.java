...
import static org.Kajeka.Graph.GraphRenderer3DFinalVariables.*;
...
import static org.Kajeka.Graph.Camera.CameraUI.GraphAnaglyphGlassesTypes.*;
import static org.Kajeka.Models.Lathe3D.Lathe3DShapeAngleIncrements.*;
...
* @see org.Kajeka.Graph.GraphRenderer3D
...
public class Graph extends GLCanvas implements GraphInterface
{
    ...
    */
    public void colorCycle(float[] color)
    {
        ...
        if (decreaseRed)
            red -= org.Kajeka.StaticLibraries.Random.getRandomRange(0, 2);
        else
            red += org.Kajeka.StaticLibraries.Random.getRandomRange(0, 2);
        ...
        if (decreaseGreen)
            green -= org.Kajeka.StaticLibraries.Random.getRandomRange(0, 2);
        else
            green += org.Kajeka.StaticLibraries.Random.getRandomRange(0, 2);
        ...
        if (decreaseBlue)
            blue -= org.Kajeka.StaticLibraries.Random.getRandomRange(0, 2);
        else
            blue += org.Kajeka.StaticLibraries.Random.getRandomRange(0, 2);
    ...
    */
    private BufferedImage loadImage(String resourceName, boolean isLocalResource)
    {
        ...
        {
            if (DEBUG_BUILD) org.Kajeka.DebugConsole.ConsoleOutput.print("MalformedURLException in Graph.loadImage() method\n" + malExc.getMessage());
    ...
    */
    ...
    {
        if (DEBUG_BUILD) org.Kajeka.DebugConsole.ConsoleOutput.print(resourceName + " transparency: ");

        switch(transparency)
        {
            case Transparency.OPAQUE:
                if (DEBUG_BUILD) org.Kajeka.DebugConsole.ConsoleOutput.print("opaque");
                break;
            case Transparency.BITMASK:
                if (DEBUG_BUILD) org.Kajeka.DebugConsole.ConsoleOutput.print("bitmask");
                break;
            case Transparency.TRANSLUCENT:
                if (DEBUG_BUILD) org.Kajeka.DebugConsole.ConsoleOutput.print("translucent");
                break;
            default:
                if (DEBUG_BUILD) org.Kajeka.DebugConsole.ConsoleOutput.print("unknown");
                break;
        }
    }
    ...
    public void printGraph()
    {
        if (USE_NEW_DESKTOP_PRINTING_FEATURE)
        {
            try
            {
                ...
                org.Kajeka.StaticLibraries.ImageProducer.writeBufferedImageToFile(getBufferedImage(), "png", tempGraphFile);
