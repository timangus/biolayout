...
public class ShaderTextureSFXs
{
    ...
    */
    public void spotCircleEffectInit(float spotCircleTimerUpdateStep, float spotCircleWidth, float spotCircleHeight, float spotCircleCenterX, float spotCircleCenterY, float distStep, boolean withNoiseEffect, int distRatio)
    {
        ...
        distMAX = (float)org.Kajeka.StaticLibraries.Math.findMinMaxFromNumbers(distMaxCornerTopLeft, distMaxCornerTopRight, distMaxCornerBottomLeft, distMaxCornerBottomRight, false); // false for MAX distance!
    ...
    */
	...
	while (--i >= 0)
	{
            waterDiv1[i] = (float)( 512.0 + org.Kajeka.StaticLibraries.Random.getRandomRange(0, 1023) );
            waterDiv2[i] = (float)( 512.0 + org.Kajeka.StaticLibraries.Random.getRandomRange(0, 1023) );
            waterMul1[i] = waterBufferWidth / 4 + org.Kajeka.StaticLibraries.Random.getRandomRange(0, waterBufferWidth / 4);
            waterMul2[i] = waterBufferHeight / 4 + org.Kajeka.StaticLibraries.Random.getRandomRange(0, waterBufferHeight / 4);
	}
    ...
    */
            ...
            int[] waterBuffer, int[] waterBuffer1, int[] waterBuffer2)
    {
        ...
        else
        {
            ...
            if ( ( (i == minNumberOfWaterDrops) && !invertStep ) || ( (i == MAX_NUMBER_OF_WATER_DROPS - 1) && invertStep ) )
            {
                ...
                i = org.Kajeka.StaticLibraries.Random.getRandomRange(minNumberOfWaterDrops, MAX_NUMBER_OF_WATER_DROPS / 4);
            }
        }
	...
	while (--i >= 0)
        ...
        if ( org.Kajeka.StaticLibraries.Random.nextBoolean() )
        {
            i = org.Kajeka.StaticLibraries.Random.getRandomRange(0, 8);
            while (--i >= 0)
            {
                px = waterBufferWidth / 16 + org.Kajeka.StaticLibraries.Random.getRandomRange( 0, (int)(waterBufferWidth / 1.125) );
                py = waterBufferHeight / 16 + org.Kajeka.StaticLibraries.Random.getRandomRange( 0, (int)(waterBufferHeight / 1.125) );
                calculateDrop(px, py, waterDropSize, waterBufferWidth, waterBufferHeight, waterBuffer1);
            }
        }
    ...
    */
    public void bumpEffectInit(GL2 gl, BufferedImage bumpImage, Texture bumpTexture, boolean bumpRandomEmboss, float bumpTimeStep)
    {
        ...
        else
        {
            ...
            bumpImage = org.Kajeka.StaticLibraries.ImageProducer.cloneImage(bumpImage);
            ...
            while (--i >= 0)
                randomBuffer[i] = org.Kajeka.StaticLibraries.Random.getRandomRange(0, 127);
    ...
    */
    static Texture createRandomEmbossTexture(GL2 gl, Texture bumpEmbossTexture, int textureUnit, int bumpWidth, int bumpHeight, int bumpBufferSize, boolean useRandomRGBColors) // package acess
    {
        ...
        int i = bumpBufferSize, j = 0;
        while (--i >= 0)
            bumpEmbossBitmapBuffer[i] = org.Kajeka.StaticLibraries.Random.getRandomRange(0, 255);
        ...
        int rs = (useRandomRGBColors) ? org.Kajeka.StaticLibraries.Random.getRandomRange(0, 7) : 0;
        int gs = (useRandomRGBColors) ? org.Kajeka.StaticLibraries.Random.getRandomRange(0, 7) : 0;
        int bs = (useRandomRGBColors) ? org.Kajeka.StaticLibraries.Random.getRandomRange(0, 7) : 0;
        int r = 0;
        int g = 0;
        int b = 0;
        i = bumpBufferSize;
