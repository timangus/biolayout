...
*
* @see org.biolayout.Physics.Vector
* @see org.biolayout.Physics.ParticlesGenerator
...
public final class Particle
{
    ...
    */
    public static ArrayList<Particle> createParticleExplosion(int numberOfParticles, Vector position, double speedMultiplier, double gravity, float life, float lifeDecrease)
    {
        ...
        {
            velocityX = speedMultiplier * (double)(org.biolayout.StaticLibraries.Random.getRandomRange(0, RAND_MAX) - RAND_MAX / 2) / (RAND_MAX / 2); // between -1.0 & 1.0
            velocityY = speedMultiplier * (double)(org.biolayout.StaticLibraries.Random.getRandomRange(0, RAND_MAX) - RAND_MAX / 2) / (RAND_MAX / 2); // between -1.0 & 1.0
            accelerationX = 0.0;
    ...
    */
    public static ArrayList<Particle> createParticleImpactExplosion(int numberOfParticles, Vector position, Vector velocity, double speedmul, int degrees, double gravity, float life, float lifeDecrease)
    {
        ...
        for (int i = 0; i < numberOfParticles; i++)
        {
            ...
            sprand = speedmul * ( 0.5 + ( (double)org.biolayout.StaticLibraries.Random.getRandomRange(0, RAND_MAX) / (double)(2 * RAND_MAX) ) ); // sprand = speed random, between 0.5 - 1.0
