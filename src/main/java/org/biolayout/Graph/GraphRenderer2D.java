...
import org.biolayout.GPUComputing.GLSL.Animation.*;
import org.biolayout.Graph.ActiveRendering.*;
import org.biolayout.Graph.GraphElements.*;
import org.biolayout.Network.*;
import org.biolayout.Physics.*;
import org.biolayout.Textures.*;
import org.biolayout.StaticLibraries.*;
import static org.biolayout.Graph.Graph.*;
import static org.biolayout.Graph.GraphRendererCommonVariables.*;
import static org.biolayout.Graph.GraphRendererCommonFinalVariables.*;
import static org.biolayout.Graph.GraphRenderer2DFinalVariables.*;
import static org.biolayout.StaticLibraries.ImageProducer.*;
import static org.biolayout.StaticLibraries.Random.*;
import static org.biolayout.Textures.DrawTextureSFXs.*;
...
/**
*
* The GraphRenderer2D class is the main 2D OpenGL renderer class.
* It utilizes TexturesLoader, ImageSFXs & DrawTextureSFXs for Texture loading, effects & drawing respectively.
*
* @see org.biolayout.Textures.TexturesLoader
* @see org.biolayout.Textures.ImageSFXs
* @see org.biolayout.Textures.TextureSFXs
* @see org.biolayout.Textures.ShaderTextureSFXs
* @see org.biolayout.Textures.DrawTextureSFXs
* @see com.jogamp.opengl.GLCanvas
* @see org.biolayout.Graph.Graph
* @see org.biolayout.Graph.GraphRendererCommonVariables
* @see org.biolayout.Graph.GraphRendererCommonFinalVariables
* @see org.biolayout.Graph.GraphRenderer2DFinalVariables
...
final class GraphRenderer2D implements GraphInterface, TileRendererBase.TileRendererListener // package access
{
    ...
    *  2D OpenGL renderer related variable.
    */
    private Texture logoImageTexture = null;

    /**
    *  2D OpenGL renderer related variable.
    */
    private Texture logoImageTextureWithBorders = null;
    ...
    */
    private void startRotation()
    {
        ...
        negateUnit = org.biolayout.StaticLibraries.Random.nextBoolean();
        enableProfiler = true;
    ...
    */
    ...
    /**
    *  Loads the star image. Uses the Singleton Design Pattern along with the static variable.
    */
    private BufferedImage loadStarImage()
    {
    ...
    */
    private void prepareImageAndTextureSFXs(GL2 gl)
    {
        ...
        if (logoImageTexture != null) logoImageTexture = null;
        logoImageTexture = TextureProducer.createTextureFromBufferedImage(LOGO_IMAGE, qualityRendering);

        if (logoImageTextureWithBorders != null) logoImageTextureWithBorders = null;
        logoImageTextureWithBorders = TextureProducer.createTextureFromBufferedImage(LOGO_IMAGE_WITH_BORDERS, qualityRendering);
        ...
        if (!USE_SHADERS_PROCESS)
        {
            ...
            imageSFXs.setWaterEffectImage(LOGO_IMAGE);
            imageSFXs.plasmaEffectInit( LOGO_IMAGE.getWidth(), LOGO_IMAGE.getHeight() );
            imageSFXs.setBumpEffectImage(256, 256, 200, 425, 200, LOGO_IMAGE);
            imageSFXs.setRadialBlurEffectImage(8, LOGO_IMAGE);
            ...
            if (USE_MULTICORE_PROCESS)
            {
                ...
                imageSFXs.radialBlurEffect((int)( 0.1f * LOGO_IMAGE.getHeight() ), true, LOGO_IMAGE.getWidth() / 4, LOGO_IMAGE.getHeight() / 4);
            }
        ...
        else
        {
            ...
            shaderTextureSFXs.waterEffectInit(logoImageTextureWithBorders, 32, true, 1, 10.0f);
            shaderTextureSFXs.bumpEffectInit(gl, LOGO_IMAGE, logoImageTexture, false, 0.01f);
            shaderTextureSFXs.blurEffectInit(gl, width, height, logoImageTexture, ShaderTextureSFXsBlurStates.FULL_BLUR, qualityRendering);
    ...
    */
    private void prepareParticlesEffect()
    {
        ...
        for (int i = 0; i < NUMBER_OF_GENERATED_PARTICLE_EFFECTS; i++)
            particleImages[i] = resizeImageByGivenRatio(loadStarImage(), PARTICLE_RESIZE_VALUE, true);
    ...
    */
    private void disposeAllTextures(GL2 gl)
    {
        ...
        */
        ...
        {
            logoImageTexture = null;
        }

        if (logoImageTextureWithBorders != null)
        {
            logoImageTextureWithBorders = null;
        }
    ...
    */
    private void renderBackgroundLayer(GL2 gl)
    {
        ...
        {
            if (!USE_SHADERS_PROCESS)
                drawRotoZoomImageTexture(gl, imageSFXs.waterEffectImage(), (width - LOGO_IMAGE.getWidth()) / 2, (height - LOGO_IMAGE.getHeight()) / 2, textureTheta, textureScaleFactor, textureScaleFactor, 1.0f);
            else
                shaderTextureSFXs.waterEffectRotoZoomRender(gl, (width - LOGO_IMAGE_WITH_BORDERS.getWidth()) / 2, (height - LOGO_IMAGE_WITH_BORDERS.getHeight()) / 2, textureTheta, textureScaleFactor, textureScaleFactor, MATERIAL_OLD_LCD_STYLE_TRANSPARENCY_SHADING.get(), 24.0f);
        }
        else if ( currentLogoEffect.equals(LogoEffects.TEXTUREDISPLACEMENT_EFFECT) )
            shaderTextureSFXs.textureDisplacementEffectRotoZoomRender( gl, logoImageTextureWithBorders, (width - LOGO_IMAGE_WITH_BORDERS.getWidth()) / 2, (height - LOGO_IMAGE_WITH_BORDERS.getHeight()) / 2, textureTheta, textureScaleFactor, textureScaleFactor, MATERIAL_OLD_LCD_STYLE_TRANSPARENCY_SHADING.get() );
        else if ( currentLogoEffect.equals(LogoEffects.BUMP_EFFECT) )
        {
            if (!USE_SHADERS_PROCESS)
                drawRotoZoomImageTexture(gl, imageSFXs.bumpEffectImage(), (width - LOGO_IMAGE.getWidth()) / 2, (height - LOGO_IMAGE.getHeight()) / 2, textureTheta, textureScaleFactor, textureScaleFactor, 1.0f);
            else
                shaderTextureSFXs.bumpEffectRotoZoomRender(gl, (width - LOGO_IMAGE.getWidth()) / 2, (height - LOGO_IMAGE.getHeight()) / 2, textureTheta, textureScaleFactor, textureScaleFactor, MATERIAL_OLD_LCD_STYLE_TRANSPARENCY_SHADING.get(), 16.0f);
        }
        else if ( currentLogoEffect.equals(LogoEffects.BLUR_EFFECT) )
            shaderTextureSFXs.blurEffectRotoZoomRender(gl, (width - LOGO_IMAGE.getWidth()) / 2, (height - LOGO_IMAGE.getHeight()) / 2, textureTheta, textureScaleFactor, textureScaleFactor, MATERIAL_OLD_LCD_STYLE_TRANSPARENCY_SHADING.get(), blurSize);
        else if ( currentLogoEffect.equals(LogoEffects.RADIAL_BLUR_EFFECT) )
        {
            if (!USE_SHADERS_PROCESS)
                drawRotoZoomImageTexture(gl, imageSFXs.radialBlurEffectImage(), (width - LOGO_IMAGE.getWidth()) / 2, (height - LOGO_IMAGE.getHeight()) / 2, textureTheta, textureScaleFactor, textureScaleFactor, 1.0f);
            else
                shaderTextureSFXs.radialBlurEffectRotoZoomRender(gl, logoImageTextureWithBorders, (width - LOGO_IMAGE_WITH_BORDERS.getWidth()) / 2, (height - LOGO_IMAGE_WITH_BORDERS.getHeight()) / 2, textureTheta, textureScaleFactor, textureScaleFactor, MATERIAL_OLD_LCD_STYLE_TRANSPARENCY_SHADING.get(), 0.1f, 0.85f);
        }
        else if ( currentLogoEffect.equals(LogoEffects.PLASMA_EFFECT) )
        {
            if (!USE_SHADERS_PROCESS)
                drawRotoZoomImageTexture(gl, imageSFXs.plasmaEffectImage(), (width - LOGO_IMAGE.getWidth()) / 2, (height - LOGO_IMAGE.getHeight()) / 2, textureTheta, textureScaleFactor, textureScaleFactor, 0.6f, new Color(PLASMA_COLOR[0], PLASMA_COLOR[1], PLASMA_COLOR[2]));
            else
                shaderTextureSFXs.plasmaEffectRotoZoomRender( gl, (width - LOGO_IMAGE.getWidth()) / 2, (height - LOGO_IMAGE.getHeight()) / 2, LOGO_IMAGE.getWidth(), LOGO_IMAGE.getHeight(), textureTheta, textureScaleFactor, textureScaleFactor, 0.5f, MATERIAL_OLD_LCD_STYLE_TRANSPARENCY_SHADING.get() );
        }
