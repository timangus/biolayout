...
import org.biolayout.Graph.ActionsUI.*;
import org.biolayout.Graph.GraphElements.*;
import org.biolayout.Graph.Selection.*;
import org.biolayout.Models.Lathe3D.*;
import org.biolayout.Network.*;
import org.biolayout.StaticLibraries.*;
import org.biolayout.Environment.DataFolder;
import org.biolayout.GPUComputing.OpenGLContext.*;
import static org.biolayout.Graph.GraphRenderer3DFinalVariables.*;
...
import static org.biolayout.Graph.Camera.CameraUI.GraphAnaglyphGlassesTypes.*;
import static org.biolayout.Models.Lathe3D.Lathe3DShapeAngleIncrements.*;
...
/**
*
* The Graph class extends GLCanvas and is the placeholder for the OpenGL canvas & renderer.
* It also provides communication support for the two renderers and the main GUI system.
*
...
* @see org.biolayout.Graph.GraphRenderer3D
...
public class Graph extends GLCanvas implements GraphInterface
{
    ...
    */
    public void colorCycle(float[] color)
    {
        ...
        if (decreaseRed)
            red -= org.biolayout.StaticLibraries.Random.getRandomRange(0, 2);
        else
            red += org.biolayout.StaticLibraries.Random.getRandomRange(0, 2);
        ...
        if (decreaseGreen)
            green -= org.biolayout.StaticLibraries.Random.getRandomRange(0, 2);
        else
            green += org.biolayout.StaticLibraries.Random.getRandomRange(0, 2);
        ...
        if (decreaseBlue)
            blue -= org.biolayout.StaticLibraries.Random.getRandomRange(0, 2);
        else
            blue += org.biolayout.StaticLibraries.Random.getRandomRange(0, 2);
    ...
    */
    private BufferedImage loadImage(String resourceName, boolean isLocalResource)
    {
        ...
        {
            if (DEBUG_BUILD) org.biolayout.DebugConsole.ConsoleOutput.print("MalformedURLException in Graph.loadImage() method\n" + malExc.getMessage());
    ...
    */
    ...
    {
        if (DEBUG_BUILD) org.biolayout.DebugConsole.ConsoleOutput.print(resourceName + " transparency: ");

        switch(transparency)
        {
            case Transparency.OPAQUE:
                if (DEBUG_BUILD) org.biolayout.DebugConsole.ConsoleOutput.print("opaque");
                break;
            case Transparency.BITMASK:
                if (DEBUG_BUILD) org.biolayout.DebugConsole.ConsoleOutput.print("bitmask");
                break;
            case Transparency.TRANSLUCENT:
                if (DEBUG_BUILD) org.biolayout.DebugConsole.ConsoleOutput.print("translucent");
                break;
            default:
                if (DEBUG_BUILD) org.biolayout.DebugConsole.ConsoleOutput.print("unknown");
                break;
        }
    }
    ...
    public void printGraph()
    {
        if (USE_NEW_DESKTOP_PRINTING_FEATURE)
        {
            try
            {
                ...
                org.biolayout.StaticLibraries.ImageProducer.writeBufferedImageToFile(getBufferedImage(), "png", tempGraphFile);
