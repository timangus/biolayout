...
import org.biolayout.GPUComputing.GLSL.*;
...
import org.biolayout.Models.SuperQuadric.*;
import org.biolayout.Models.Loaders.OBJModelLoader.*;
...
import static org.biolayout.Graph.Camera.GraphCameraFinalVariables.*;
import static org.biolayout.Models.Lathe3D.Lathe3DShapeAngleIncrements.*;
import static org.biolayout.Models.ModelRenderingStates.*;
import static org.biolayout.Network.GraphmlNetworkmEPN3DShapesDefinitions.*;
...
/**
*
* The GraphRenderer3D class is the main 3D OpenGL renderer class.
*
* @see org.biolayout.Textures.Glyphbombing3DTexture
* @see org.biolayout.Textures.PerlinNoise3DTexture
* @see org.biolayout.Textures.ShaderLightingSFXs
...
* @see org.biolayout.Graph.GraphRenderer3DFinalVariables
...
final class GraphRenderer3D implements GraphInterface, TileRendererBase.TileRendererListener // package access
{
    ...
    */
    private void preparePointSprites(GL2 gl)
    {
        ...
        // Tell OpenGL to use the lower left corner for point sprite texture coordinates so as to display a given texture properly
            ...
            if (animationRender)
            {
                if ( ( ( ANIMATION_SELECTED_NODES_ANIMATION_ONLY.get() ) ? ( ( !selectionManager.getSelectedNodes().isEmpty() ) ? selectionManager.getSelectedNodes().contains(node) : true ) : true ) )
                {
                    if ( !node.ismEPNTransition() )
                    {
                        ...
                        {
                            Tuple6<Float, Color, Boolean, Float, Boolean, Float> tuple6 = AnimationVisualization.performAnimationVisualization(true, node.getNodeID(), node.getNodeName(), layoutFrame.isAllShadingSFXSValueEnabled(), nodeColor, currentTick, animationFrameCount, animationSpectrumImage, DATA_TYPE.equals(DataTypes.CORRELATION));
