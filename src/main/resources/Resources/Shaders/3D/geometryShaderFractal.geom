...
uniform bool fractalShrinkTriangles;
uniform bool fractalSolidWireFrame;
uniform bool fractalNormals;
...
void main() 
{
    ...
    float shrinkFactor;
    if (fractalShrinkTriangles)
    {
        useShrinkFactor = true;
        shrinkFactor = 0.9 + 0.05 * abs( sin(fractalTimer) );
    }
    else if (fractalMorphing != 0.0)
    {
        useShrinkFactor = true;
        shrinkFactor = 0.25 + 0.75 * abs( sin(PI / 2.0 + fractalTimer) );
    }
    else
    {
    ...
    applyTriangleGeometry(useShrinkFactor, shrinkFactor, fractalUserClipping, true);

    if (fractalSolidWireFrame && fractalNormals)
