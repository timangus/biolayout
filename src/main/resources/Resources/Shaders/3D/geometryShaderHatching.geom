...
uniform bool hatchingShrinkTriangles;
uniform bool hatchingSolidWireFrame;
uniform bool hatchingNormals;
...
void main() 
{
    ...
    float shrinkFactor;
    if (hatchingShrinkTriangles)
    {
        useShrinkFactor = true;
        shrinkFactor = 0.9 + 0.05 * abs( sin(hatchingTimer) );
    }
    else if (hatchingMorphing != 0.0)
    {
        useShrinkFactor = true;
        shrinkFactor = 0.25 + 0.75 * abs( sin(PI / 2.0 + hatchingTimer) );
    }
    else
    {
    ...
    applyTriangleGeometry(useShrinkFactor, shrinkFactor, hatchingUserClipping, hatchingTexturing);

    if (hatchingSolidWireFrame && hatchingNormals)
