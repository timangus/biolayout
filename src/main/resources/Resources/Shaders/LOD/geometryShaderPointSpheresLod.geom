...
uniform  bool pointSpheresLodShrinkTriangles;
uniform bool pointSpheresLodNormals;
...
void main() 
{
    ...
    float shrinkFactor;
    if (pointSpheresLodShrinkTriangles)
    {
        useShrinkFactor = true;
        shrinkFactor = 0.9 + 0.05 * abs( sin(pointSpheresLodTimer) );
    }
    else
    {
    ...
    applyTriangleGeometry(useShrinkFactor, shrinkFactor, pointSpheresLodTexturing);

    #if APPLY_NORMALS_GEOMETRY_CONDITION
        if (pointSpheresLodNormals)
