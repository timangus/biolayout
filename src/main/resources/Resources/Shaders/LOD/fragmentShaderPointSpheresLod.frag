...
uniform bool pointSpheresLodFog;
uniform bool pointSpheresLodTexturing;
uniform bool pointSpheresLodState;
uniform bool pointSpheresLodOldLCDStyleTransparency;
uniform bool pointSpheresLodSolidWireFrame;
...
void main()
{
    if (pointSpheresLodOldLCDStyleTransparency)
    ...
    vec4 finalColor = applyADSLightingModel(pointSpheresLodState, true, fsNormal, fsPosition, sceneColorLocal);    
    if (alpha < 1.0)
    ...
    #if GPU_GEOMETRY_SHADER4_COMPATIBILITY_CONDITION    
        if (pointSpheresLodSolidWireFrame)
            finalColor = applySolidWireFrame(finalColor, 1.5);    
    #endif 
    // apply per-pixel fog if appriopriate
    gl_FragColor = (pointSpheresLodFog) ? applyFog(finalColor) : finalColor;
}