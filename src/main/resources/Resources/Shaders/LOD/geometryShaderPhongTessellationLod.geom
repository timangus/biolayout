...
uniform  bool phongTessellationLodShrinkTriangles;
uniform bool phongTessellationLodNormals;
...
void main() 
{
    ...
    float shrinkFactor;
    if (phongTessellationLodShrinkTriangles)
    {
        useShrinkFactor = true;
        shrinkFactor = 0.9 + 0.05 * abs( sin(phongTessellationLodTimer) );
    }
    else
    {
    ...
    applyTriangleGeometry(useShrinkFactor, shrinkFactor, phongTessellationLodTexturing);

    #if APPLY_NORMALS_GEOMETRY_CONDITION
        if (phongTessellationLodNormals)
