...
uniform bool pointNormalTrianglesLodFog;
uniform bool pointNormalTrianglesLodTexturing;
uniform bool pointNormalTrianglesLodState;
uniform bool pointNormalTrianglesLodOldLCDStyleTransparency;
uniform bool pointNormalTrianglesLodSolidWireFrame;
...
void main()
{
    if (pointNormalTrianglesLodOldLCDStyleTransparency)
    ...
    vec4 finalColor = applyADSLightingModel(pointNormalTrianglesLodState, true, fsNormal, fsPosition, sceneColorLocal);    
    if (alpha < 1.0)
    ...
    #if GPU_GEOMETRY_SHADER4_COMPATIBILITY_CONDITION    
        if (pointNormalTrianglesLodSolidWireFrame)
            finalColor = applySolidWireFrame(finalColor, 1.5);    
    #endif 
    // apply per-pixel fog if appriopriate
    gl_FragColor = (pointNormalTrianglesLodFog) ? applyFog(finalColor) : finalColor;
}