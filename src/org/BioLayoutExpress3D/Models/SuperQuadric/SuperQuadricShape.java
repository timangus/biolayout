...
public class SuperQuadricShape extends ModelShape
{
    ...
    /**
    *  Variable needed to enable OOP C++ native code instead of default procedural C native code.
    *  SUPER_QUADRIC_SHAPE_COMPATIBLE_OOP is for Visual Studio 2010 compiled dlls which support only WinVista & Win7. 
    */           
    private static final boolean USE_OOP_NATIVE_CODE = false;
    ...
    */
    private void initNativeLibrary()
    {
        ...
        {
            int index = (USE_OOP_NATIVE_CODE) ? ( ( LoadNativeLibrary.isWinVista() || LoadNativeLibrary.isWin7() ) ? NativeLibrariesTypes.SUPER_QUADRIC_SHAPE_OOP.ordinal() : NativeLibrariesTypes.SUPER_QUADRIC_SHAPE_COMPATIBLE_OOP.ordinal() ) : NativeLibrariesTypes.SUPER_QUADRIC_SHAPE.ordinal();
    ...
    */     
    ...
    protected final void performCreateGeometry(GL gl)
    {
        ...
        if (USE_NATIVE_CODE && USE_OOP_NATIVE_CODE) allocateNativeResources(vertices, normals, texCoords, superQuadricSettings.superQuadricShapeType.ordinal(), modelSettings.usingNormals, modelSettings.usingTexCoords,
                                                                            superQuadricSettings.a1, superQuadricSettings.a2, superQuadricSettings.a3, superQuadricSettings.alpha, superQuadricSettings.n, superQuadricSettings.e, superQuadricSettings.u1, superQuadricSettings.u2, superQuadricSettings.v1, superQuadricSettings.v2, superQuadricSettings.uSegments, superQuadricSettings.vSegments, superQuadricSettings.s1, superQuadricSettings.s2, superQuadricSettings.t1, superQuadricSettings.t2);
    ...
    */
    private void createGeometry(float dU, float dV, float dS, float dT, int startPosition, int endPosition) 
    {
        if (USE_NATIVE_CODE)
        {
            if (USE_OOP_NATIVE_CODE)
                createGeometryNative(dU, dV, dS, dT, startPosition, endPosition);
            else            
                createGeometryNative(vertices, normals, texCoords, superQuadricSettings.superQuadricShapeType.ordinal(), modelSettings.usingNormals, modelSettings.usingTexCoords,
                                     superQuadricSettings.a1, superQuadricSettings.a2, superQuadricSettings.a3, superQuadricSettings.alpha, superQuadricSettings.n, superQuadricSettings.e, superQuadricSettings.u1, superQuadricSettings.u2, superQuadricSettings.v1, superQuadricSettings.v2, superQuadricSettings.uSegments, superQuadricSettings.vSegments, superQuadricSettings.s1, superQuadricSettings.s2, superQuadricSettings.t1, superQuadricSettings.t2,
                                     dU, dV, dS, dT, startPosition, endPosition);
        }
        else
            createGeometryJava(dU, dV, dS, dT, startPosition, endPosition);        
    }    
    
    /*
    *  Allocates all native resources (native C++ method).
    */    
    private native void allocateNativeResources(float[] vertices, float[] normals, float[] texCoords, int superQuadricShapeType, boolean usingNormals, boolean usingTexCoords,
                                                float a1, float a2, float a3, float alpha, float n, float e, float u1, float u2, float v1, float v2, int uSegments, int vSegments, float s1, float s2, float t1, float t2);    
    
    /*
    *  Generates a solid SuperQuadrics using the parameters from superQuadricSettings and optionally generates normals & texture coordinates (native C++ method).
    */
    private native void createGeometryNative(float dU, float dV, float dS, float dT, int startPosition, int endPosition);    
    
    /*
    *  Generates a solid SuperQuadrics using the parameters from superQuadricSettings and optionally generates normals & texture coordinates (native C method).
    */
    private native void createGeometryNative(float[] vertices, float[] normals, float[] texCoords, int superQuadricShapeType, boolean usingNormals, boolean usingTexCoords,
                                             float a1, float a2, float a3, float alpha, float n, float e, float u1, float u2, float v1, float v2, int uSegments, int vSegments, float s1, float s2, float t1, float t2,
                                             float dU, float dV, float dS, float dT, int startPosition, int endPosition);      
    
    /*
    *  Releases all native resources (native C++ method).
    */     
    private native void releaseNativeResources();
    
    /*
    *  Generates a solid SuperQuadrics using the parameters from superQuadricSettings and optionally generates normals & texture coordinates (Java method).
    */
    private void createGeometryJava(float dU, float dV, float dS, float dT, int startPosition, int endPosition) 
    {
    ...
    */
    ...
    /*
    *  Tests to see if point P<x,y,z> is inside the SuperQuadric superQuadricSettings (Java method).
    ...
    */    
    public float insideOut(float x, float y, float z)
    {
        ...
        {
            if (USE_OOP_NATIVE_CODE)
                return insideOutNative(x, y, z);
            else
                return insideOutNative(superQuadricSettings.a1, superQuadricSettings.a2, superQuadricSettings.a3, superQuadricSettings.n, superQuadricSettings.e, superQuadricSettings.alpha, superQuadricSettings.superQuadricShapeType.ordinal(),
                                       x, y, z);
        }
        else
            return insideOutJava(x, y, z);
    }     
    
    /*
    *  Tests to see if point P<x,y,z> is inside the SuperQuadric superQuadricSettings (native C++ method).
    *  Returns 1 if on the surface, > 1 if outside the surface, or
    *  < 1 if inside the surface
    */    
    private native float insideOutNative(float x, float y, float z);    
    ...
    *  < 1 if inside the surface
    */    
    private native float insideOutNative(float a1, float a2, float a3, float n, float e, float alpha, int superQuadricShapeType,
    ...
    *  < 1 if inside the surface
    */    
    private float insideOutJava(float x, float y, float z)
    {
        if ( superQuadricSettings.superQuadricShapeType.equals(ELLIPSOID) )
